    ...
    public:
        ...
        struct boss_vanessa_vancleefAI : public BossAI
        {
            ...
            void Reset() OVERRIDE
            {
                ...
                if (instance)
                {
                    instance->DoUpdateWorldState(WORLDSTATE_VANCLEEF_VINDICATION, 1);
            ...
            void JustDied(Unit* /*killer*/) OVERRIDE
            {
                ...
                if (!players.isEmpty())
                {
                    if (Group* group = players.begin()->GetSource()->GetGroup())
                        if (group->isLFGGroup())
                            sLFGMgr->FinishDungeon(group->GetGUID(), 326);
                }
            }
        ...
        {
            return GetDeadminesAI<boss_vanessa_vancleefAI>(creature);
        }
};
...
class npc_vanessa_vancleef_vanessa_sitting : public CreatureScript
{
    public:
                ...
                if (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        ...
                        case EVENT_INTRO_7:
                            ...
                            instance->DoStartTimedAchievement(ACHIEVEMENT_TIMED_TYPE_EVENT, ACHIEVEMENT_EVENT);
        ...
        CreatureAI* GetAI(Creature* creature) const OVERRIDE
        {
            return GetDeadminesAI<npc_vanessa_vancleef_vanessa_sittingAI>(creature);
        }
};
...
class npc_vanessa_vancleef_defias_enforcer : public CreatureScript
{
        public:
            ...
            CreatureAI* GetAI(Creature* creature) const OVERRIDE
            {
                return GetDeadminesAI<npc_vanessa_vancleef_defias_enforcerAI>(creature);
            }
};
