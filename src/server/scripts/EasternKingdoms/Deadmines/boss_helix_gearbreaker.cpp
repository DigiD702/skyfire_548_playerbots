                ...
                while (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        ...
                        case EVENT_HELIX_CREW:
                            for (uint8 i = 0; i < 4; i++)
                                me->SummonCreature(NPC_HELIX_CREW, helixcrewPos[i], TempSummonType::TEMPSUMMON_CORPSE_DESPAWN);
                            break;
                    }
                }
...
class npc_lumbering_oaf : public CreatureScript
{
    public:
                    ...
                    switch (eventId)
                    {
                        ...
                        case EVENT_CHARGE_OAF3:
                            ...
                            for (uint32 i = 0; i < 9; ++i)
                            {
                                ...
                                if (Creature* rat = me->SummonCreature(NPC_MINE_RAT, src, TempSummonType::TEMPSUMMON_TIMED_OR_DEAD_DESPAWN, 60000))
        ...
        CreatureAI* GetAI(Creature* creature) const OVERRIDE
        {
            return GetDeadminesAI<npc_lumbering_oafAI>(creature);
        }
};
...
class npc_sticky_bomb : public CreatureScript
{
    public:
            ...
            uint64 GetTarget()
            {
                ...
                for (ThreatContainer::StorageType::const_iterator i = threatlist.begin(); i != threatlist.end(); ++i)
                    if (Unit* target = ObjectAccessor::GetUnit(*me, (*i)->getUnitGuid()))
                        if (target->GetTypeId() == TypeID::TYPEID_PLAYER && me->GetExactDist2dSq(target) < 4 && !target->GetVehicle())
        ...
        CreatureAI* GetAI(Creature* creature) const OVERRIDE
        {
            return GetDeadminesAI<npc_sticky_bombAI>(creature);
        }
};
            ...
            void UpdateAI(uint32 diff) OVERRIDE
            {
                ...
                while (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        case EVENT_STICKY_BOMB:
                            if (Unit* target = SelectTarget(SELECT_TARGET_RANDOM, 0, 0.0f, true))
                                me->SummonCreature(NPC_STICKY_BOMB, target->GetPositionX(), target->GetPositionY(), target->GetPositionZ(), 0.0f, TempSummonType::TEMPSUMMON_TIMED_DESPAWN, 30000);
