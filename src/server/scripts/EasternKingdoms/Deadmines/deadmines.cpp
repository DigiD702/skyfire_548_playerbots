...
class npc_deadmines_prototype_reaper : public CreatureScript
{
public:
    npc_deadmines_prototype_reaper() : CreatureScript("npc_deadmines_prototype_reaper") { }

    struct npc_deadmines_prototype_reaperAI : public ScriptedAI
    {
        npc_deadmines_prototype_reaperAI(Creature* creature) : ScriptedAI(creature) { }

        void DamageTaken(Unit* /*attacker*/, uint32& /*damage*/) OVERRIDE
        {
            if (HealthBelowPct(90))
                if (InstanceScript* instance = me->GetInstanceScript())
                    instance->DoUpdateWorldState(WORLDSTATE_PROTOTYPE_PRODIGY, 0);
        }
    ...
    {
        return GetDeadminesAI<npc_deadmines_prototype_reaperAI>(creature);
    }
};
...
{
    new npc_deadmines_prototype_reaper();
