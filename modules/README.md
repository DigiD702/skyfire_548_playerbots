...
worldserver without editing the core. The design mirrors the AzerothCore module
system so that AzerothCore modules can be ported with minimal structural changes.
...
character replaced by `_`. For example `mod-playerbots` must implement
`Addmod_playerbotsScripts()`. Inside it, call your own `AddSC_*` registration
functions, e.g.:

```cpp
void AddSC_my_feature();          // defined in your module

void Addmod_playerbotsScripts()   // discovered + invoked automatically
{
    AddSC_my_feature();
}
```
